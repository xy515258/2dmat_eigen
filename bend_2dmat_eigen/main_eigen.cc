...
///////////////////////////////////////////////////////////////////////////////////////////////
void calc_dFdw(double * dFdw, double * kappa, double * w, double ** dw, fftw_complex * kbend_1, fftw_complex * kbend_2, double **** lam, double *** eps, double ** epsbar, double *** sig00, double ** kxy, ptrdiff_t local_n0, ptrdiff_t N0, ptrdiff_t N1)
...
///////////////////////////////////////////////////////////////////////////////////////////////
{
    ...
    for (int j=0; j<N1c; j++)
    {
        ...
        kdFdw[ndx][Re] =  kxy[X][ndx]*ktemp0[ndx][Im] + kxy[Y][ndx]*ktemp1[ndx][Im] + (k4x + k4y)*0.22/*kappa[ndx]*/*kw[ndx][Re] + kbend_2[ndx][Re];
        kdFdw[ndx][Im] = -kxy[X][ndx]*ktemp0[ndx][Re] - kxy[Y][ndx]*ktemp1[ndx][Re] + (k4x + k4y)*0.22/*kappa[ndx]*/*kw[ndx][Im] + kbend_2[ndx][Im]; 
    }
...
void calc_inhom_eps00(double *** inhom_eps00, double **** lam, double ***** delta_S, double *** eps00, double **** eps0, double ** eta, ptrdiff_t local_n0, ptrdiff_t N1)
{    
    const int N1r = 2*(N1/2+1);     
    ...
    for (int j=0; j<N1; j++)
     ...
     { 
       ...
       for (int kk=0; kk<2; kk++)
       for (int ll=0; ll<2; ll++)
          inhom_eps00[ii][jj][ndx] += (-1)*lam[ii][jj][kk][ll]*eps00[kk][ll][ndx];

       for (int kk=0; kk<2; kk++)
       for (int ll=0; ll<2; ll++)
       for (int p=0; p<3; p++)
          inhom_eps00[ii][jj][ndx] += (1)*lam[ii][jj][kk][ll]*eps0[p][kk][ll][ndx]*eta[p][ndx]*eta[p][ndx];
       ...
       for (int tt=0; tt<2; tt++) 
          inhom_eps00[ii][jj][ndx] += (1)*lam[ii][jj][kk][ll]*fabs(delta_S[kk][ll][mm][nn][ndx])*lam[mm][nn][ss][tt]*eps00[ss][tt][ndx];
       ...
       for (int tt=0; tt<2; tt++) 
       for (int pp=0; pp<3; pp++)
            inhom_eps00[ii][jj][ndx] += (-1)*lam[ii][jj][kk][ll]*fabs(delta_S[kk][ll][mm][nn][ndx])*lam[mm][nn][ss][tt]*eps0[pp][ss][tt][ndx]*eta[pp][ndx]*eta[pp][ndx];
        ...
        while (change_etap_max > ip.change_etap_thresh)
        {
          ...
          while (change_eps00_max > ip.change_eps00_thresh)
           { 
            ...
            calc_inhom_eps00(inhom_eps00, lam, delta_S, eps00, eps0, eta, local_n0, N1);
            ...
            calc_dFdw(dFdw, kappa, w, dw, kbend_1, kbend_2, lam, eps, epsbar, sig00, kxy, local_n0, N0, N1);
