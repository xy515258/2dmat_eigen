...
void calc_chemical_potential(double *** sig00, double **** eps0, double ** chem, double ** eta, double *** eps00, 
...
double max_ku ( fftw_complex ** data, int local_n0, int N1)
{
      double m = 0;
    for (int i=0; i<local_n0; i++)
    for (int j=0; j<N1/2+1; j++)
    {
        int ndx = i*(N1/2+1) + j;
        for (int ii=0; ii<2; ii++)
       { if (fabs(data[ii][ndx][Re]) > m) m = fabs(data[ii][ndx][Re]);
         if (fabs(data[ii][ndx][Im]) > m) m = fabs(data[ii][ndx][Im]);
     }}
    return m;
}
...
int main(int argc, char ** argv)
{
    ...
    pf.unpack("mu_el2", ip.mu_el2);
    pf.unpack("nu_el2", ip.nu_el2);
    ...
    calc_elastic_tensors(lam, eps0, sig0, sigeps, ip.mu_el1, ip.nu_el1, local_n0, N1, delta_S, ip.mu_el2, ip.nu_el2, phi);
    ...
    for (int step=1; step<=ip.nsteps; step++)
    {
        ...
        while (change_etap_max > ip.change_etap_thresh)
        {
          ...
          while (change_eps00_max > ip.change_eps00_thresh)
           ...
           // myfile << "ku_max = " << max_ku(ku, local_n0, N1) << std::endl;
        ...
        if (step % ip.out_freq == 0) {
            ...
            output("inhom_eps00/"+zeroFill(frame), inhom_eps00[1][1], N0, N1, local_n0);
            output("inhom_eps00_1/"+zeroFill(frame), inhom_eps00_1[1][1], N0, N1, local_n0);
            output("inhom_eps00_2/"+zeroFill(frame), inhom_eps00_2[1][1], N0, N1, local_n0);
